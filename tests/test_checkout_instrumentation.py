...
class _Billing:
    ...
    async def calculate_price(
        ...
        channel_discount: dict[str, Any] | None = None,
    ) -> SimpleNamespace:
        self.calls.append(
            {
...
class _ChannelIncentiveService:
    def __init__(self, context: dict[str, Any] | None = None) -> None:
        self.context = context

    async def discount_context(self, user, **kwargs) -> dict[str, Any] | None:
        return self.context
...
async def test_duration_getter_uses_experiment_pricing_context():
    ...
    result = await raw(
        dm,
        make_user(telegram_id=777),
        make_i18n(),
        billing,
        exp_service,
        _ChannelIncentiveService(),
    )
...
async def test_on_payment_method_select_passes_experiment_context_to_create_payment():
    ...
    payment_call = next(call for call in billing.calls if call["method"] == "create_payment")
...
@pytest.mark.asyncio
async def test_on_payment_method_select_passes_channel_discount_to_billing():
    plan = _plan_with_durations(plan_id=42, days=[30])
    dm = make_dialog_manager()
    dm.middleware_data[USER_KEY] = make_user(telegram_id=777)
    dm.dialog_data["plandto"] = plan.model_dump()
    ...
    billing = _Billing()
    notification_service = AsyncMock()
    exp_service = _ExperimentService()
    channel_context = {
    ...
    await raw(
        ...
        billing,
        exp_service,
        _ChannelIncentiveService(channel_context),
        notification_service,
        redis_client,
    )

    payment_call = next(call for call in billing.calls if call["method"] == "create_payment")
    assert payment_call["channel_discount"] == channel_context
...
async def test_on_payment_method_select_does_not_pass_start_price_for_annual_duration():
    ...
    payment_call = next(call for call in billing.calls if call["method"] == "create_payment")
    assert payment_call["duration_days"] == 365
    assert payment_call["experiment"] is None
